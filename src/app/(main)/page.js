          ...
          {Array.from({ length: 3 }).map((_, index) => (
            ...
            >
              ...
              <p className="mt-2 font-semibold">
                Được tạo bởi: Người dùng {index + 1}
              </p>
            </div>
          ))}
        </div>
        ...
        <div className="bg-white p-4 rounded-lg shadow mt-10 transition-transform duration-200 hover:shadow-xl">
          ...
          <ul className="mt-4">
            {["Bài viết", "Ghi chú", "Thống kê"].map((option, index) => (
              <li
                key={index}
                className="flex justify-between items-center py-2 border-b border-gray-200 transition duration-200 hover:bg-blue-50 hover:text-blue-600"
              >
