...
export default function ReportApp() {
  ...
  useEffect(() => {
    const checkLoginStatus = () => {
      ...
      if (loggedIn) {
        ...
        // Set empty state when not logged in
        setPosts([]);
        setDemos([]);
        setUserPosts([]);
        setUserDemos([]);
  ...
  const maxSystemCount = Math.max(publishedCount, draftCount, 1); // Avoid division by zero
  const systemStepSize = maxSystemCount < 10 ? 1 : maxSystemCount < 100 ? 5 : 10;
  const maxUserCount = Math.max(userPublishedCount, userDraftCount, 1); // Avoid division by zero
              ...
              <>
                <div className="mb-6 bg-gray-50 p-4 rounded-lg shadow-inner">
                  ...
                  <ul className="space-y-2 text-gray-700">
                    <li>Tổng bài viết: <span className="font-bold">{isLoggedIn ? posts.length + demos.length : 0}</span></li>
                    <li>Bài của bạn: <span className="font-bold">{isLoggedIn ? userPosts.length + userDemos.length : 0}</span></li>
                    <li>Năm khả dụng: <span className="font-bold">{isLoggedIn ? availableYears.length : 0}</span></li>
                  </ul>
                </div>
                  ...
                  <div className="bg-green-100 p-4 rounded-lg flex items-center justify-between hover:scale-105 transition duration-200">
                    <div className="flex items-center">
                      ...
                      <div>
                        ...
                        <p className="text-2xl font-bold text-green-600">{isLoggedIn ? userPublishedCount : 0}</p>
                      </div>
                    </div>
                  </div>
                ...
                {isUserFiltered && isLoggedIn ? (
                      ...
                      <table className="w-full border-collapse bg-white rounded-lg text-sm">
                        ...
                        <tbody>
                          {filteredUserPosts.concat(filteredUserDemos).length > 0 ? (
                            ...
                            <tr>
                              <td colSpan="3" className="border-b border-gray-200 px-4 py-2 text-center text-gray-600">
                                Không có dữ liệu
                              </td>
                            </tr>
                          )}
                  ...
                  !isLoggedIn && (
                    <div className="mt-6 text-gray-600">Vui lòng đăng nhập để xem kết quả lọc.</div>
                  )
                )}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                  <div className="bg-green-100 p-4 rounded-lg flex items-center justify-between hover:scale-105 transition duration-200">
                    <div className="flex items-center">
                      ...
                      <div>
                        <p className="text-gray-700">Đã đăng</p>
                        <p className="text-2xl font-bold text-green-600">{isLoggedIn ? publishedCount : 0}</p>
                      </div>
                    </div>
                  </div>
                  <div className="bg-gray-100 p-4 rounded-lg flex items-center justify-between hover:scale-105 transition duration-200">
