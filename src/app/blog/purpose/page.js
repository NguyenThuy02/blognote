...
import { EditOutlined, ZoomInOutlined, ZoomOutOutlined, CloseOutlined, DownloadOutlined } from '@ant-design/icons';
...
export default function PurposePage() {
          ...
          if (typeof purpose.images === 'string') {
            if (isJsonString(purpose.images)) {
              try {
                ...
                console.error(`Lỗi phân tích JSON hình ảnh cho mục đích ${purpose.id}:`, e);
                images = [];
              }
  ...
  const renderMilestones = (milestones) => {
    ...
    return milestones.map((milestone, index) => (
      ...
      >
        ...
        <p className="mt-2 text-gray-800 font-medium text-sm sm:text-base">{milestone.description}</p>
      </div>
    ));
  };
  ...
  const renderQuestions = (questions, purposeId, purposeImages) => {
    ...
    return questions.map((q, index) => {
      ...
      return (
        <div key={`question-${purposeId}-${index}`} className="mb-4 sm:mb-6 p-3 sm:p-4 bg-blue-50 rounded-lg shadow-sm border border-gray-100">
          {imageSrc && renderImage(imageSrc, `Hình ảnh câu hỏi ${index}`, index, purposeId)}
          <p className="font-bold text-base sm:text-lg text-gray-900">{q.question || 'Câu hỏi không có nội dung'}</p>
  ...
  const renderQuizzes = (quizzes, purposeId, purposeImages) => {
    ...
    return quizzes.map((quiz, index) => {
      ...
      return (
        <div key={`quiz-${purposeId}-${index}`} className="mb-4 sm:mb-6 p-3 sm:p-4 bg-blue-50 rounded-lg shadow-sm border border-gray-100">
          {imageSrc && renderImage(imageSrc, `Hình ảnh câu đố ${index}`, index, purposeId)}
          <p className="font-bold text-base sm:text-lg text-gray-900">{quiz.question || 'Câu đố không có nội dung'}</p>
      ...
      <div className="mt-3 sm:mt-4 border border-gray-200 rounded-lg bg-gray-50">
        <div className="flex items-center justify-between p-2 sm:p-3 cursor-pointer" onClick={toggleExpand}>
          <div className="flex items-center">
            <svg className="w-5 sm:w-6 h-5 sm:h-6 text-gray-500 mr-1 sm:mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z" />
            </svg>
            <span className="text-xs sm:text-sm text-gray-700">{fileName}</span>
          </div>
          <div className="flex gap-1 sm:gap-2">
            ...
            <svg
              className={`w-4 sm:w-5 h-4 sm:h-5 text-gray-500 transform transition-transform duration-200 ${isExpanded ? 'rotate-180' : ''}`}
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
          ...
          <div className="p-2 sm:p-3 border-t border-gray-200">
            {fileExtension === 'pdf' ? (
              ...
              <div
                ...
                dangerouslySetInnerHTML={{ __html: docxContent[`${purposeId}-${fileName}`] || 'Đang tải nội dung...' }}
              />
            ) : (
  ...
  const renderPurpose = (purpose) => {
    ...
    return (
      <div key={purpose.id} className="relative">
        <div
          className="rounded-lg shadow-md border bg-white border-gray-200 p-3 sm:p-4 hover:shadow-[0_0_15px_rgba(0,0,0,0.2)] transition-shadow duration-200"
        >
          <div className="flex items-center mb-2 sm:mb-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">{new Date(purpose.created_at).toLocaleDateString('vi-VN')}</p>
            </div>
          </div>

          <h2 className="text-lg sm:text-xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-purple-400 mb-2">
...
purpose.poll.multipleChoice ? 'Chọn nhiều đáp án' : 'Chọn một đáp án'}
            ...
            <div className="mt-4 sm:mt-6 p-3 sm:p-4 bg-blue-50 rounded-lg shadow-sm border border-gray-100">
                ...
                <>
                  {Array.isArray(purpose.images) && purpose.images.length > 0 && (
                    <div className="mb-3 sm:mb-4">
                      <h4 className="text-base sm:text-lg font-bold text-gray-800 mb-2 sm:mb-3">Hình ảnh truyện tranh</h4>
              ...
              {purpose.story_type === 'Truyện chữ' && (
                <>
                  {Array.isArray(purpose.images) && purpose.images.length > 0 && (
                    <div className="mb-3 sm:mb-4">
                      <h4 className="text-base sm:text-lg font-bold text-gray-800 mb-2 sm:mb-3">Hình ảnh truyện chữ</h4>
                      <div className="space-y-3 sm:space-y-4">
                        {purpose.images.map((img, index) => (
                  ...
                  {purpose.story_content && (
                    <div className="mt-3 sm:mt-4">
                      <h4 className="text-base sm:text-lg font-bold text-gray-800 mb-2 sm:mb-3">Nội dung truyện:</h4>
  ...
  return (
    <div
      className={`text-gray-700 mt-[97px] shadow-2xl border border-blue-300 rounded-lg min-h-screen flex flex-col ${getThemeClasses(theme, 'editor')}`}
    >
      <div
        className={`flex-1 mx-2 sm:mx-4 pt-4 sm:pt-5 pb-4 sm:pb-5 my-2 sm:my-3 rounded-lg shadow-md ${getThemeClasses(theme, 'container')}`}
      >
        ...
        <div
          ...
          className="px-2 sm:px-4 flex flex-col md:flex-row md:gap-4 scrollbar-hidden max-h-[calc(100vh-200px)] sm:max-h-[calc(100vh-220px)] overflow-y-auto"
        >
          <div className="flex-1 flex flex-col gap-3 sm:gap-4">
            {filteredPurposes.map(purpose => renderPurpose(purpose))}
          </div>
          <div className="hidden md:flex md:flex-1 md:flex-col md:gap-4">
            {filteredPurposes.slice(Math.ceil(filteredPurposes.length / 2)).map(purpose => renderPurpose(purpose))}
          </div>
        </div>
      </div>
      ...
      {showLoginModal && (
        <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50">
          <div className="bg-white p-4 sm:p-6 rounded-lg max-w-sm sm:max-w-md w-full">
            <h3 className="text-base sm:text-lg font-bold text-gray-800 mb-3 sm:mb-4">Yêu cầu đăng nhập</h3>
            <p className="text-gray-600 text-xs sm:text-sm mb-4 sm:mb-6">Vui lòng đăng nhập để thực hiện hành động này.</p>
