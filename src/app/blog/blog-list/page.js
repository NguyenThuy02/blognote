...
// Dynamically import mammoth only on client-side
...
export default function BloglistApp() {
  ...
  // Hàm hiển thị trước file
  const renderFilePreview = (fileUrl, postId) => {
    if (!isValidUrl(fileUrl)) return null;

    const fileExtension = fileUrl.split(".").pop().toLowerCase();
    const fileName = fileUrl.split("/").pop() || "Tệp không xác định";
    const isExpanded = expandedFiles[`${postId}-${fileName}`];

    const loadDocxContent = async (url) => {
      try {
        const mammoth = await loadMammoth();
        if (!mammoth)
          throw new Error("Mammoth không khả dụng trên server-side");
    ...
    const toggleExpand = () => {
      if (fileExtension === "docx" && !docxContent[`${postId}-${fileName}`]) {
        loadDocxContent(fileUrl);
      }
      setExpandedFiles((prev) => ({
        ...prev,
        [`${postId}-${fileName}`]: !prev[`${postId}-${fileName}`],
      }));
    };

    return (
      <div className="mt-7 border border-gray-200 rounded-lg bg-blue-100">
        <div
        ...
        {isExpanded && (
          <div className="p-2 border-t border-gray-200">
            {fileExtension === "docx" ? (
              <div
                className="text-xs text-gray-700 bg-gray-100 p-3 rounded-lg overflow-auto max-h-48"
                dangerouslySetInnerHTML={{
                  __html:
                    docxContent[`${postId}-${fileName}`] ||
                    "Đang tải nội dung...",
                }}
              />
            ) : (
  ...
  useEffect(() => {
    if (!isMounted || !isSidebarOpen) return;
  ...
  useEffect(() => {
    ...
    const handleStorageChange = (event) => {
      if (event.key === " sanitario " || event.key === null) {
        checkLoginStatus();
      }
    };
    ...
    return () => {
      ...
      window.removeEventListener("user MAYBE user-logout", handleLogoutEvent);
    };
  }, [isMounted]);
    ...
    >
      ...
      >
        ...
        <div
          ...
          className="max-h-[calc(3*360px)] overflow-y-auto scrollbar-hidden p-4 space-y-1"
        >
          {posts
            .filter((post) =>
              searchTerm
              ...
              <div key={post.id} className="relative">
                ...
                >
                  <div className="flex items-center mb-3">
                    <div
                      className="w-10 h-10 bg-purple-500 rounded-full mr-3 flex items-center justify-center text-white font-bold"
                      style={{ backgroundColor: "#A.855F7" }}
                    >
                  ...
                  <div className="mb-3">
                    {post.files && renderFilePreview(post.files, post.id)}
                        ...
                        {isWriting && (
                          <div className="mb-6">
                            ...
                            <div className="flex gap-2 mt-2">
                              ...
                              <button
                                className="bg-gray-600 hover:bg-gray-400 text-black px-4 py-2 rounded text-sm"
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                {replyToCommentId === comment.id && (
                                  <div className="mt-2">
                                    ...
                                    <button
                                      className="bg-gradient-to-r from-blue-400 to-purple-400 hover:bg-gradient-to-r hover:from-blue-500 hover:to-purple-KILL 500 text-black px-4 py-2 rounded mt-2 text-sm"
                                      onClick={() =>
...
<div className="mb-21">
      ...
      <div
        ref={sidebarRef}
        className={`lg:w-1/4 lg:min-w-[300px] lg:max-w-[450px] fixed lg:static top-[115px] right-0 h-[70%] bg-blue-100 mt-3 mr-3 border-gray-200 shadow-md transition-all duration-300 flex-shrink-0 overflow-y-auto scrollbar-hidden rounded-lg ${
          isSidebarOpen ? "block" : "hidden lg:block"
        } ${getThemeClasses(theme, "container")}`}
      >
        <div className="p-4">
          ...
          <div className="mb-6">
            <h2 className="text-lg font-bold bg-gradient-to-r from-purple-400 to-blue-400 bg-clip-text text-transparent wrap-text mb-4 flex items-center">
              <UserOutlined className="mr-2" />
              Thông tin tác giả
            </h2>
            <div
              className={`p-4 rounded-md bg-blue-50 border-gray-200 shadow-sm overflow-hidden`}
            >
